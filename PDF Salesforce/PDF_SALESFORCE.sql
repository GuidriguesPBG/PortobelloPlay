CREATE OR REPLACE PACKAGE BODY APPS.PDF_SALESFORCE AS
/*******************************************************************************
* Logiciel : PL_FPDF                                                           *
* Version :  0.1                                                               *
* Date :     11/03/2022                                                        *
* Auteur :   Guilherme Rodrigues                                               *
********************************************************************************
...
type tranform_matrix  is table of float index by  binary_integer;
...
-- Returns the Linespacing property
--------------------------------------------------------------------------------
function GetCellMargin return number is
begin
    -- Get LineSpacing property
    return cmargin;
end GetCellMargin;
...
-- sets the Linespacing property
--------------------------------------------------------------------------------
Procedure SetCellMargin (pls number) is
begin
    -- Set LineSpacing property
    cmargin := pls;
end SetCellMargin;
...
begin
 ...
 procedure lob_replace(psearch varchar2,
                       preplace varchar2,
                       psubject in out nocopy clob) is
   n number;
 begin
   n := dbms_lob.instr(psubject, psearch);
   if (nvl(n, 0) > 0) then
     dbms_lob.copy(psubject,
                   psubject,
                   dbms_lob.getlength(psubject),
                   n + length(preplace),
                   n + length(psearch));

     dbms_lob.write(psubject, length(preplace), n, preplace);
     if (length(psearch) > length(preplace)) then
       dbms_lob.trim(psubject,
                     dbms_lob.getlength(psubject) -
                     (length(psearch) - length(preplace)));
     end if;
   end if;
 end;
...
begin
    ...
    if(state = 2) then
       if pages(page) is null then
         dbms_lob.createtemporary(pages(page),false);
         l_offset:=1;
       else
          l_offset:= dbms_lob.getlength(pages(page))+1;
       end if;

       dbms_lob.write(pages(page),l_amount,l_offset,pstr || lv_CRLF);
    else
      if pdfDoc is  null then
         dbms_lob.createtemporary(pdfDoc,false);
         l_offset:=1;
      else
         l_offset:= dbms_lob.getlength(pdfDoc)+1;
      end if;

      dbms_lob.write(pdfDoc,l_amount, l_offset, pstr || lv_CRLF);
    end if;
exception
  when others then
   -- bug('p_out : '||sqlerrm);
...
procedure p_out(pstr clob default null, pCRLF boolean default true) is
  lv_CRLF varchar2(2) := chr(10);
begin
    -- Add a line to the document
    if(state = 2) then
        dbms_lob.append(pages(page),pstr );
        if  pCRLF then
           dbms_lob.append(pages(page),lv_CRLF) ;
        end if;
    else
      dbms_lob.append(pdfDoc,pstr );
      if  pCRLF then
           dbms_lob.append(pdfDoc,lv_CRLF) ;
        end if;
    end if;
exception
  when others then
...
procedure p_outSaveGraphicsState is
begin
  p_out('q');

end;
procedure p_outRestoreGraphicsState is
begin
  p_out('Q');
end;
----------------------------------------------------------------------------------------
...
begin
   -- Replace number of pages
     if not empty(AliasNbPages) then
           for i in 1..nb loop
             lob_replace(AliasNbPages,nb,pages(i));
...
function getPDF return blob is
...
begin
   ...
   if  pdfDoc is not null then
      v_len := dbms_lob.getlength(pdfDoc);
      v_in := 1;
      v_out := 1;
      v_lang := 0;
      v_warning := 0;
      dbms_lob.convertToBlob(v_doc,
                             pdfDoc,
                             v_len,
                             v_in,
                             v_out,
...
procedure StartTransform is
begin
    if state != 2 then
            return;
    end if;
  p_outSaveGraphicsState;
end;
procedure Transform(ptm tranform_matrix) is
begin
  p_out(to_char(ptm(0),'fm999999990D00','NLS_NUMERIC_CHARACTERS = ''.,''')||' '||
        to_char(ptm(1),'fm999999990D00','NLS_NUMERIC_CHARACTERS = ''.,''')||' '||
        to_char(ptm(2),'fm999999990D00','NLS_NUMERIC_CHARACTERS = ''.,''')||' '||
        to_char(ptm(3),'fm999999990D00','NLS_NUMERIC_CHARACTERS = ''.,''')||' '||
        to_char(ptm(4),'fm999999990D00','NLS_NUMERIC_CHARACTERS = ''.,''')||' '||
        to_char(ptm(5),'fm999999990D00','NLS_NUMERIC_CHARACTERS = ''.,''')||' cm');
end;
procedure Rotate(pangle number, px number default null,py number default null) is
  lx number;
  ly number;
  ltm tranform_matrix;
begin
  ...
  ly := (h - ly) * k;
  lx := lx*k;
  ltm(0):=cos(degree2radian(pangle));
  ltm(1):=sin(degree2radian(pangle));
  ltm(2):= -ltm(1);
  ltm(3):=ltm(0);
  ltm(4):= lx+(ltm(1)*ly) -(ltm(0)*lx);
  ltm(5):= ly-(ltm(0)*ly) -(ltm(1)*lx);
  Transform(ltm);
end;
procedure StopTransform is
begin
  if state != 2 then
            return;
    end if;
  p_outRestoreGraphicsState;
end;
----------------------------------------------------------------------------------------
