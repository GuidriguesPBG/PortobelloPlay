...
l_msg_index_out              number(10);
  ...
  type order_header_r is record(cust_po_number               varchar2(20)   -- [obrigatorio] - Numero do pedido de compra do cliente PBG
                               ...
                               ,sold_from_fiscal_name        varchar2(50)   -- [Opcional] - Nome da organização da qual o cliente está adquirindo seus produtos em sua ordem de compra
                               ,order_source                 varchar2(30)   -- Aplicacao de integracao - usado para reconhecimento em tratativas particulares
                               ,source_hdr_id                varchar2(50)   -- Chave de identificacao da aplicacao de integracao a nivel de cabecalho
  ...
  type order_line_r is record(line_number              number        -- [opcional] - Numero da linha na ordem de compra. Usar contador caso nao seja informado
                             ...
                             ,promo_pricebook_prefix   varchar2(30) -- Prefixo da tabela promocional usada no item
                             );
  --
  ...
  procedure insert_log (p_source_hdr_id    in varchar2
  ...
  function read_log (p_source_hdr_id    in varchar2
                    ,p_source_lin_id    in number
                    ,p_log_type         in number
                    ) return varchar2;
  --
  ...
  procedure p_vldt_insert_sites(p_header        in out order_header_r
                               ,p_header_aux    in out order_header_aux_r
                               ,p_return_status    out varchar2
                               ,p_log_processo     out varchar2);
  --
  ...
  --
    ...
    FUNCTION valida_qtd_camada (p_inventory_item_id IN NUMBER,p_sku               in varchar2,p_camada            IN VARCHAR2,p_qtd               in number) RETURN VARCHAR2;
...
/*    If trim(p_header.request_date) is null then -- deve permmitir não enviar a DMF
    ...
    cursor c_json is
      ...
      from json_table (p_body,'$.dados[*]'
                        columns(order_source                 varchar2 path '$.order_source'
                               ,source_hdr_id                varchar2 path '$.source_hdr_id'
  ...
  Begin
    ...
    for i in 1..rec_order_json.count loop
      ...
      If v_return_status = 'S' and v_tem_endereco then
        p_vldt_insert_sites(rec_order_json(i)                           ,l_order_header_aux                           ,v_return_status                           ,v_log_processo);
      End If;
      */
      --
  ...
  cursor c1 is
    select 1
    from   XXPB_ORDER_API_SALESFORCE_LOGS
    where  1=1
  ...
  begin
    ...
    if v_insere_msg then
      --
      begin
        insert into XXPB_ORDER_API_SALESFORCE_LOGS(source_hdr_id     --number        not null
      ...
      exception
        when others then
          insert_log(p_source_hdr_id,p_source_lin_id,'Erro: 1 - Erro na insercao de log na tabela XXPB_ORDER_API_SALESFORCE_LOGS. Erro: '||sqlerrm||dbms_utility.format_error_backtrace, 2);
      end;
      --
      commit;
      --
...
*/      --
   ...
   --procedure main(p_body           in varchar2,
      ...
      select cust_po_number
       ...
       from dmfmin,
            json_table (p_body,'$' columns (cust_po_number               varchar2   path '$.cust_po_number'
                                           ...
                                           ,sold_from_fiscal_name        varchar2   path '$.sold_from_fiscal_name' 
    ...
    cursor c_order_line is
      select nvl(line_number,rownum) line_number
            ...
            ,ordered_quantity
            ,unit_of_measure
            ,substr(ship_from_org_code,1,3) as ship_from_org_code
       ...
       from json_table (p_body,'$.items[*]' columns (line_number              number       path '$.line_number'
                                                    ...
                                                    ,promo_pricebook_prefix   varchar2(30) path '$.promo_pricebook_prefix' 
                                                    )
                       )
  ...
  begin
    ...
    insert into XXPB_PO_SALESFORCE_CTRL (order_source, source_hdr_id)
    ...
    begin
      --
      delete from XXPB_ORDER_API_SALESFORCE_LOGS
    ...
    exception
      when others then
        l_ds_erro := 'Erro: 1 - Erro ao excluir log do processamento, XXPB_ORDER_API_SALESFORCE_LOGS. Erro: '||sqlerrm||dbms_utility.format_error_backtrace;
        raise w_erro;
    end;
    --
    -- Seleciona a origem para transacao a ser gerada
    ...
    if nvl(l_order_header.sold_from_fiscal_id , ' ') = ' ' then 
        if upper(l_order_header.sold_from_fiscal_name) = 'PORTOBELLO' then 
            l_order_header.sold_from_fiscal_id := '83475913000272';
        end if;
        if upper(l_order_header.sold_from_fiscal_name) = 'POINTER' then 
            l_order_header.sold_from_fiscal_id := '20465785000185';
        end if;
...
*/    
    ...
    -- Criação de Local de entrega 
    if nvl(l_order_header.cust_ind_type,'X') != 'X' and nvl(l_order_header.cust_address2,'X') != 'X' and nvl(l_order_header.cust_zip_code,'X') != 'X' THEN 
        p_vldt_insert_sites(l_order_header
                           ,l_order_header_aux
                           ,v_return_status
                           ,v_log_processo);
                           
        if v_return_status = 'E' then 
              l_ds_erro := 'Erro: 7.0 - Criação de endereço de entrega' || v_log_processo;
              raise w_erro;
        end if;
    end if;
    ...
    if x_sts_code <> c_status_sucesso then
      ...
      if x_om_order_num is null then
        ...
        delete from XXPB_PO_SALESFORCE_CTRL
         where order_source = l_order_header.order_source
           and source_hdr_id = l_order_header.source_hdr_id;
        commit;
      end if;
  ...
  cursor c1(p_inventory_item_code in varchar2
    ...
    cursor c3(p_prefix varchar2) is 
    SELECT LIST_HEADER_ID
        FROM (SELECT DISTINCT A1.NAME Name, A.LIST_HEADER_ID
              FROM APPS.QP_LIST_HEADERS_B A,
                   APPS.QP_LIST_HEADERS_TL A1,
                   apps.ra_salesreps_all B
             WHERE     A.LIST_HEADER_ID = A1.LIST_HEADER_ID
                   AND INSTR (
                          B.ATTRIBUTE5,
                          TRIM (TRANSLATE (A1.NAME, '1234567890', '          '))) >
                          0
                   AND TRUNC (SYSDATE) BETWEEN TRUNC (
                                                  NVL (B.start_date_active,
                                                       SYSDATE))
                                           AND TRUNC (
                                                  NVL (B.end_date_active,
                                                       SYSDATE))
                   AND B.ATTRIBUTE5 IS NOT NULL
                   AND B.org_id = fnd_profile.VALUE ('ORG_ID')
                   AND B.ATTRIBUTE4 IN
                          (SELECT LOVA.lookup_code
                            ...
                            WHERE     LOVA.ENABLED_FLAG = 'Y'
                                  ...
                                  AND LOVA.LOOKUP_TYPE =
                                         'ONT_CANAL_VENDA_LIBERADO_IVOP')
                   AND A.ATTRIBUTE1 IS NOT NULL                    -- PRAZO TABELA
                   AND A.ATTRIBUTE3 IS NOT NULL                   -- TAXA DE JUROS
                   AND A.END_DATE_ACTIVE IS NOT NULL
                   AND A.LIST_TYPE_CODE = 'PRL'
                   AND A.ACTIVE_FLAG = 'Y'
                   AND TRUNC (SYSDATE) BETWEEN TRUNC (
                                                  NVL (A.START_DATE_ACTIVE,
                                                       SYSDATE))
                                           AND TRUNC (
                                                  NVL (A.END_DATE_ACTIVE,
                                                       SYSDATE))
  ...
  begin
    ...
    for i in p_items.first..p_items.last loop
      ...
      v_promo_price_list_id := 0;
     
     
     if p_items(i).promo_pricebook_prefix is not null then 
         open c3(p_items(i).promo_pricebook_prefix);
         fetch c3 into r3;
         if c3%found then 
            v_promo_price_list_id := r3.list_header_id;
         end if;
         if nvl(v_promo_price_list_id,0) > 0 then 
           v_price_list_id_cliente:= v_promo_price_list_id;
         end if;
     end if;
  ...
  procedure book_order ( p_source_hdr_id    in varchar2
 ...
 wmsg_data        varchar2(2000);
  ...
  wreturn_status   varchar2(1);    
    ...
    --Seleciona dos dados de comissao
    CURSOR c_order_comiss (p_header_id IN NUMBER) IS
      ...
      if o_return_status = fnd_api.g_ret_sts_success then
        ...
        FOR r_comiss IN (select val_percent, rep_id
         ...
         --Busca dados da comissao ja criada
          OPEN c_order_comiss(x_om_header_id);
          FETCH c_order_comiss
          INTO w_sales_credit_id,
               w_created_by,
               w_creation_date,
               w_salesrep_id;
          CLOSE c_order_comiss;

          IF w_salesrep_id = r_comiss.rep_id THEN
            l_achou_registro := 1;
          ...
          END IF;
          l_header_scredit_tbl(l_header_scredit_index).orig_sys_credit_ref   := p_header.source_hdr_id;
          l_header_scredit_tbl(l_header_scredit_index).header_id             := x_om_header_id;
    ...
    END LOOP;
        ...
        if nvl(l_sts_sales_credit,'ZZZ') = fnd_api.g_ret_sts_success then
          ...
          FOR i IN 1 .. l_msg_count LOOP
            oe_msg_pub.get(i, fnd_api.g_false, l_msg_data, l_msg_index_out);
            insert_log(p_header.source_hdr_id, '', '- Erro retornado: - Achou ' || l_achou_registro || ' - ' || l_msg_data||' '||l_msg_index_out , 2);
          END LOOP;
          rollback;
        END IF;
...
--                       ,p_converte            => l_converte
        ...
        IF (UPPER(vg_item_classificacao) = ITEM_CLASSIFICACAO_COMERCIAL) THEN
          ...
          IF valida_qtd_camada(vg_inventory_item_id, p_items(i).sku, p_items(i).camada ,p_items(i).ordered_quantity) <> RETORNO_VALIDO THEN
            x_sts_code := c_status_erro;
            insert_log(p_header.source_hdr_id, p_items(i).line_number, 'Erro: 2 - Rotina de validacao de linhas - '||i||', item: '||p_items(i).inventory_item_code||'. '||MSG_ERRO, 2);
          END IF;
  ...
  begin
    ...
    for i in p_items_aux.first..p_items_aux.last loop
      ...
      if p_items_aux(i).validate_price = false then
        ...
        insert_log(p_header.source_hdr_id,null,'Aviso: Divergencia de preço: '|| 'Linha '||i||': '||p_items_aux(i).desc_error||chr(10),1);
  ...
  --
    ...
    p_qtd_camada number;
    begin
        --p_inventory_item_id := 46148;
        --p_sku := '000017';
        --p_camada := '17';
        --p_qtd := 285.6;
        
        SELECT CASE
                  WHEN SUBSTR (NLS_UPPER (MTL.PRIMARY_UOM_CODE), 1, 2) = 'PC'
                  THEN
                       (SELECT A28.QTD_M2_CAIXA QTD_M2_CAIXA
                          FROM (SELECT A97.INVENTORY_ITEM_ID INVENTORY_ITEM_ID,
                                       A96.SEGMENT1 SEGMENT1,
                                       A96.PRIMARY_UOM_CODE PRIMARY_UOM_CODE,
                                       DECODE (
                                          A97.FROM_UOM_CODE,
                                          'm2', ROUND (
                                                     A98.CONVERSION_RATE
                                                   * A97.CONVERSION_RATE,
                                                   3),
                                          'kg', ROUND (A98.CONVERSION_RATE, 3),
                                          'pc', ROUND (
                                                     A98.CONVERSION_RATE
                                                   * (1 / A97.CONVERSION_RATE),
                                                   3),
                                          'jg', ROUND (
                                                     A98.CONVERSION_RATE
                                                   * A97.CONVERSION_RATE,
                                                   3),
                                          'un', DECODE (
                                                   A96.PRIMARY_UOM_CODE,
                                                   'jg', ROUND (A98.CONVERSION_RATE,
                                                                3),
                                                   ROUND (
                                                        A98.CONVERSION_RATE
                                                      * (1 / A97.CONVERSION_RATE),
                                                      3)),
                                          'm3', ROUND (A98.CONVERSION_RATE, 3))
                                          QTD_M2_CAIXA
                                  FROM INV.MTL_UOM_CONVERSIONS A98,
                                       INV.MTL_UOM_CLASS_CONVERSIONS A97,
                                       INV.MTL_SYSTEM_ITEMS_B A96
                                 WHERE     A97.INVENTORY_ITEM_ID =
                                              A98.INVENTORY_ITEM_ID
                                       AND A97.INVENTORY_ITEM_ID =
                                              A96.INVENTORY_ITEM_ID
                                       AND A97.TO_UOM_CODE =
                                              DECODE (A96.PRIMARY_UOM_CODE,
                                                      'm2', 'un',
                                                      'pc', 'm2',
                                                      'kg', 'un',
                                                      'kg')
                                       AND A97.CONVERSION_RATE > 0
                                       AND A98.UOM_CODE = 'cx'
                                       AND A96.ORGANIZATION_ID =
                                              APPS.PB_MASTER_ORGANIZATION_ID ()) A28
                         WHERE A28.INVENTORY_ITEM_ID = MTL.INVENTORY_ITEM_ID)
                     * (  APPS.INV_CONVERT.INV_UM_CONVERT (MTL.INVENTORY_ITEM_ID,
                                                           0,
                                                           1,
                                                           'pa',
                                                           'cx',
                                                           'pallet (pa)',
                                                           'caixa')
                        / DECODE (APPS.INV_CONVERT.INV_UM_CONVERT (
                                  ...
                                  APPS.INV_CONVERT.INV_UM_CONVERT (
                                     ...
                                     'pallet (pa)',
                                     'camada (ca)')))
                  WHEN SUBSTR (NLS_UPPER (MTL.PRIMARY_UOM_CODE), 1, 2) = 'M2'
                  THEN
                       APPS.INV_CONVERT.INV_UM_CONVERT (MTL.INVENTORY_ITEM_ID,
                                                        0,
                                                        1,
         ...
         WHERE INVENTORY_ITEM_ID = p_inventory_item_id AND ORGANIZATION_ID = pb_master_organization_id;
                
         IF nvl(to_number(p_camada),0) <= 0 THEN
            RETURN RETORNO_VALIDO;
         END IF;
         
         IF NVL(p_qtd_camada,0) * nvl(to_number(p_camada),0) <> nvl(p_qtd,0) then 
            MSG_ERRO := 'Qtd. inválida para o número de camadas! Qtd. em camadas: ' || NVL(p_qtd_camada,0) * nvl(to_number(p_camada),0) || ' | Qtd. Solicitada: ' || p_qtd;
            RETURN RETORNO_INVALIDO;
         --   DBMS_OUTPUT.PUT_LINE('Qtd. não bateu : Total Camada' || NVL(p_qtd_camada,0) * nvl(to_number(p_camada),0) || ' | Total Digitado' || p_qtd); 
         --else
         --   DBMS_OUTPUT.PUT_LINE('Qtd. bateu!');
         end if; 
        
           
         IF TO_NUMBER(SUBSTR(p_sku, LENGTH(p_sku)-1,2)) <> to_number(p_camada) THEN
            MSG_ERRO := 'Camada digitada inválida para este SKU! SKU: ' || p_sku || ' | Camada: ' || p_camada ;
            RETURN RETORNO_INVALIDO;
         --      DBMS_OUTPUT.PUT_LINE('Camada inválida');
         --ELSE
         --   DBMS_OUTPUT.PUT_LINE('Camada válida');
         END IF;
