CREATE OR REPLACE PACKAGE APPS.PB_INTEGRA_SALESFORCE is
  -- +=================================================================+
  ...
  -- |                                                                 |
  -- | DESCRIPTION                                                     |
  -- |   Objetivo deste objeto e suportar a interface entre o modulo de|
  -- |   pedido do software Ivop e o modulo Order Management da solucao|
  -- |   solucao Oracle release 11.5.10.                               |
  -- |                                                                 |
  -- | CREATED BY                                                      |
  -- |   RSerafim            REVISION 11.1         07/04/2012          |
  -- |                                                                 |
  -- | UPDATED BY                                                      |
  -- |                                                                 |
  -- +=================================================================+
...
CREATE OR REPLACE PACKAGE BODY APPS.PB_INTEGRA_SALESFORCE is
  -- Procedure para validar se quantidade informada referesse a caixa aberta ou fechada
  procedure p_gera_dados_cli_po(errbuf  OUT  VARCHAR2, errcode OUT  VARCHAR2, p_completa IN NUMBER) is
  --
/******************************************************************************
...
******************************************************************************/
    ...
    SELECT MIN(OHA.ORDERED_DATE) FIRST_DATE
     ...
     AND (
        (NVL(P_EMPRESA,0) = 1 AND OHA.SALES_CHANNEL_CODE IN('1','2','4','23','11','25','5','7'))
        OR
        (NVL(P_EMPRESA,0) <> 1 AND OHA.SALES_CHANNEL_CODE IN('101','102','105'))
        ) 
    ...
    CURSOR c_qtd (p_account_id IN NUMBER, p_empresa IN NUMBER) IS 
    -- Dados gerais do clientes e contabilização dos dados de quantidade de pedidos nos ultimos 24meses
    ...
    SELECT OHA.ORDER_TYPE_ID, OHA.ORDER_NUMBER, OHA.PAYMENT_TERM_ID, OHA.SHIP_TO_ORG_ID, OHA.SALES_CHANNEL_CODE, OHA.SHIP_FROM_ORG_ID,
           COND.NAME, OHA.ORDERED_DATE, SALES_CHANNEL_CODE, shipping_method_code, shipment_priority_code, T.carrier_id
      FROM OE_ORDER_HEADERS_ALL OHA
           INNER JOIN RA_TERMS COND ON OHA.PAYMENT_TERM_ID = COND.TERM_ID  
       LEFT JOIN WSH_CARRIER_SERVICES T ON T.ship_method_code = shipping_method_code
     WHERE OHA.CANCELLED_FLAG = 'N' 
     AND OHA.ORDER_CATEGORY_CODE = 'ORDER' 
...
BEGIN
    ...
    for p in (SELECT ACCO.CUST_ACCOUNT_ID
                 ...
                 AND OHA.ORDER_CATEGORY_CODE = 'ORDER' 
                 AND (NVL(p_completa,0) = 1 OR (NVL(p_completa,0) = 0 AND OHA.ORDERED_DATE >= nvl(v_last_exec,sysdate -7)))
                 AND OHA.SALES_CHANNEL_CODE IN('1','2','101','102','105','23','11','25','5','7')
                 AND ACCO.STATUS = 'A'
	    ...
	    v_last_channel := null;
            ...
            if v_first_order is not null then 
                ...
                FETCH c_last_po
                INTO v_order_type, v_order_number, v_default_payment, v_endereco, v_channel, v_warehouse, v_descr_payment, v_last_order, v_last_channel, v_shipping_method_code, v_shipment_priority_code, v_carrier_id;
                CLOSE c_last_po;
               ...
               if v_qtd_dias > 365 then
                    ...
                    IF v_qtd_pedido between 36 and 249 then
                ...
                if v_qtd_dias between 183 and 365 then
                ...
                QTD_OV, QTD_DAY, BRAND, STATUS,PROCESSED, SALES_CHANNEL, SHIPPING_METHOD_CODE, SHIPMENT_PRIORITY_CODE, CARRIER_ID) VALUES 
                ...
                v_qtd_pedido, v_qtd_dias, 'Portobello','A',1, v_last_channel, v_shipping_method_code, v_shipment_priority_code, v_carrier_id);
                COMMIT;
	    ...
	    v_last_channel := null;
            ...
            if v_first_order is not null then 
               ...
               if v_qtd_dias > 365 then
                ...
                v_last_header_id, v_last_order, v_last_invoice, v_last_payment, v_first_order,
                v_warehouse, v_warehouse_descr, v_puchase_frequency,
                v_qtd_pedido, v_qtd_dias, 'Pointer', 'A', 1, v_last_channel, v_shipping_method_code, v_shipment_priority_code, v_carrier_id);
                COMMIT;
    ...
    UPDATE APPS.XXPB_HIST_PO_CLIENT SET QTD_DAY = TRUNC(SYSDATE - LAST_ORDER) WHERE NVL(PROCESSED,0) = 0;
